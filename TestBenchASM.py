...
def instruction_to_machine_code(instruction,ln):
    parts = instruction.split()
    opcode = OPCODES[parts[0]]
    
    if parts[0] == "disp":
        register = REGISTERS[parts[1]]  # Remove the comma
        return f"{opcode}{register}000",
    elif parts[0] == "add":
        ...
        return f"{opcode}{register}000", immediate
    elif parts[0] == "sub":
        register1 = REGISTERS[parts[1][:-1]]  # Remove the comma
        register2 = REGISTERS[parts[2]]
        return f"{opcode}{register1}{register2}",
...
def generate_testbench(instruction, ln):
    codes = instruction_to_machine_code(instruction, ln+1)
    tb = []
    if not (codes == "//UNKNOWN INSTRUCTION"):
...
if __name__ == "__main__":
    ...
    testbench_code = ["""`timescale 1ns/1ns
/*
...
wire [3:0] tick;
wire [15:0] bus, R0, R1, R2, R3, R4, R5, R6, R7, display;
reg [15:0] bus_prev, R0_prev, R1_prev, R2_prev, R3_prev, R4_prev, R5_prev, R6_prev, R7_prev, display_prev;

ext_proc myProcessor(clock, rst, din, bus, tick, R0, R1, R2, R3, R4, R5, R6, R7, display);\n\n"""]
    ...
    for ln, instruction in enumerate(instructions):
        ...
        if not instruction or instruction.startswith(';'):  # Check if the line is empty or a comment
            continue
        testbench_code.append(generate_testbench(instruction, ln))
    ...
    with open('testbench.txt', 'w') as output:
        output.write('\n'.join(testbench_code[:]))

print("Testbench code written to testbench.txt")
